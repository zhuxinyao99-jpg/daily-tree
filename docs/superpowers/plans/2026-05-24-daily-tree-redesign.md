# Daily Tree Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Redesign Daily Tree with 7-stage tree growth system, horizontal timeline nav strip, clickable branch history, and landing page flash fix.

**Architecture:** `tree.js` is a full rewrite — 7 stage canvas renderers + `growBranch` that populates a `branchSegments[]` array returned from `drawTree`; `app.js` consumes those segments for hit detection and adds timeline strip logic; HTML/CSS gets the timeline strip and nav restructure.

**Tech Stack:** Canvas 2D, vanilla JS ES modules, localStorage, no build step.
...
| File | Change |
|------|--------|
| `index.html` (root) | 1-line flash fix |
| `app/tree.js` | Full rewrite — 7 stages, new trunk/branch/crown system, returns `{ branchSegments }` |
| `app/index.html` | Remove search button, add timeline strip HTML, update nav |
| `app/style.css` | Timeline strip styles, nav update, "今天" chip |
| `app/app.js` | selectedDate state, timeline render, new `onTreeClick` using branchSegments |

---

## Task 1: Landing Page Flash Fix

**Files:**
- Modify: `index.html` (root) — line 4

- [ ] **Step 1: Remove meta-refresh, add JS redirect**

Open `index.html`. Replace line 4:
```html
<meta http-equiv="refresh" content="0;url=landing.html">
```
With (keep the `<title>` and other metas, just remove the refresh meta and add a script at the top of `<head>` before any other tags):
```html
<script>window.location.replace('landing.html');</script>
```
The script goes as the very first child of `<head>`, before all other tags.

- [ ] **Step 2: Verify**

Open `index.html` in browser → should jump to landing.html with no white flash.

- [ ] **Step 3: Commit**
```bash
git add index.html
git commit -m "fix: replace meta-refresh with JS redirect to eliminate landing flash"
```

---

## Task 2: Rewrite tree.js — Core Utilities

**Files:**
- Modify: `app/tree.js` — replace entire file

Start with a fresh file. This task writes the foundation: RNG, ground, roots, trunk, branchSeg, growBranch, crown. The stage renderers come in Task 3.

- [ ] **Step 1: Write the new tree.js skeleton with utilities**

Replace the entire contents of `app/tree.js` with:
...
function drawCrown(ctx, cx, cy, radius, rng) {
  ...
  // individual leaves — 3 passes (back→front)
...
```bash
cd /Users/ericlu/DailyTree/app && node --input-type=module < tree.js 2>&1 | head -5
```
Expected: no output (no top-level side effects yet). If error, fix before continuing.

---

## Task 3: Rewrite tree.js — Stage Renderers + Exports

**Files:**
- Modify: `app/tree.js` — append after the utilities from Task 2

- [ ] **Step 1: Append stage renderers**

Add the following to the end of `app/tree.js`:
...
// ── Stage: Crack (1 day) — seed with visible split ───────────────────────────
function drawCrack(ctx, W, H) {
  // draw the seed first
  drawSeed(ctx, W, H);
  const cx = W / 2, cy = H * 0.62;
  // crack line from center top of seed
...
function drawSapling(ctx, W, H, seed) {
  ...
  // 2 branches — simple lines (no growBranch yet)
  const br2 = rngFrom(seed + 10);
  const tips = [], segs = [];
  growBranch(ctx, cx + 1, gy - 65, -Math.PI / 2 + 0.45, 32, 6, 1, br2, tips, segs, 'early');
  growBranch(ctx, cx + 1, gy - 65, -Math.PI / 2 - 0.45, 30, 5.5, 1, rngFrom(seed + 20), tips, segs, 'early');
  tips.forEach(pt => {
    const cr = rngFrom(Math.floor(pt.x * 11 + pt.y * 17 + seed));
    drawCrown(ctx, pt.x, pt.y, 22, cr);
  });
}
...
function drawYoung(ctx, W, H, seed) {
  ...
  tips.forEach(pt => {
    const cr = rngFrom(Math.floor(pt.x * 11 + pt.y * 17 + seed));
    drawCrown(ctx, pt.x, pt.y, 28, cr);
  });
}

// ── Shared: draw mature/ancient with explicit branchDefs ─────────────────────
function drawFullTree(ctx, W, H, cfg, seedN, segments) {
  ...
  branchDefs.forEach(def => {
    ...
    const br2 = rngFrom(Math.floor(bx * 7 + by * 13 + seedN));
    growBranch(ctx, bx, by, def.ang + (rng() - 0.5) * 0.08, blen, bw, cfg.branchDepth, br2, tips, segments, def.period);
  });

  tips.forEach(pt => {
    const cr2 = rngFrom(Math.floor(pt.x * 11 + pt.y * 17 + seedN));
    const r = cfg.trunkH * (0.18 + cr2() * 0.10);
    drawCrown(ctx, pt.x, pt.y, r, cr2);
  });
}
...
// seed: integer — use (firstEntryYear * 1000 + dayCount) or any stable int
...
// Animated variant for seed/sprout (oscillating glow + sway)
// t = DOMHighResTimeStamp from requestAnimationFrame
...
- [ ] **Step 2: Append animated seed/crack/sprout functions**

Add these before the `export function getStage` line (they're used by `drawAnimated`):
...
```bash
cd /Users/ericlu/DailyTree/app && node --input-type=module --eval "import { getStage, drawTree } from './tree.js'; console.log(getStage(0), getStage(1), getStage(2), getStage(8), getStage(31), getStage(91), getStage(301));"
```
Expected output: `seed crack sprout sapling young mature ancient`

- [ ] **Step 4: Remove old getTrunkBounds export (no longer needed)**

The old `getTrunkBounds` function is removed in this rewrite since hit detection now uses `branchSegments`. Make sure it's not in the file.

- [ ] **Step 5: Commit**
```bash
git add app/tree.js
git commit -m "feat: rewrite tree.js — 7-stage renderer with explicit trunk/branch/crown system"
```

---

## Task 4: Timeline Strip HTML + CSS

**Files:**
- Modify: `app/index.html` — bottom-bar section
- Modify: `app/style.css` — timeline + nav styles

- [ ] **Step 1: Update bottom nav HTML**

In `app/index.html`, replace the entire `<nav class="bottom-bar" ...>` block (lines 45–70) with:

```html
<!-- ── Timeline Strip ── -->
<div class="timeline-strip" id="timeline-strip" role="navigation" aria-label="日期导航"></div>

<!-- ── Bottom Action Bar ── -->
<nav class="bottom-bar" id="bottom-bar">
  <button class="bar-icon" id="btn-stats" aria-label="统计">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
      <rect x="3" y="12" width="4" height="9"/><rect x="10" y="7" width="4" height="14"/><rect x="17" y="3" width="4" height="18"/>
    </svg>
    <span class="bar-icon-label">统计</span>
  </button>
  ...
  <button class="bar-icon" id="btn-settings" aria-label="设置">
    ...
    <span class="bar-icon-label">设置</span>
  </button>
</nav>
```

Also remove the two `<div class="bar-chart" ...>` elements and the search button from anywhere else they appear.

- [ ] **Step 2: Add timeline strip + nav CSS to style.css**

Add to the end of `app/style.css`:
...
.timeline-strip {
  ...
  gap: 4px;
  padding: 0 16px;
  background: linear-gradient(to top, rgba(6,10,6,0.96), rgba(6,10,6,0.80));
...
.timeline-cell:active { opacity: 0.7; }
.timeline-cell.today {
  background: rgba(74,170,80,0.22);
  border: 1px solid rgba(74,170,80,0.40);
}
.timeline-cell.selected:not(.today) {
...
.timeline-cell.today .timeline-day,
.timeline-cell.selected .timeline-day { color: #6fcf5a; }
.timeline-cell.has-entry .timeline-day { color: rgba(255,255,255,0.65); }
...
.timeline-cell.has-entry .timeline-dot { background: rgba(74,170,80,0.70); }
.timeline-cell.today .timeline-dot { background: #6fcf5a; }
...
.today-chip {
  ...
  top: 60px;
  right: 16px;
  background: rgba(74,170,80,0.20);
  border: 1px solid rgba(74,170,80,0.45);
  color: #6fcf5a;
  font-size: 12px;
  font-weight: 500;
  padding: 5px 12px;
...
.today-chip.visible { display: block; }

/* Updated bottom bar — 3 items */
.bottom-bar {
  display: flex;
  align-items: center;
...
Also update `.bottom-bar` height reference. Find the existing `.bottom-bar` rule in style.css and ensure it has:
```css
height: 56px;
```

And update `canvas-wrap` bottom padding to account for timeline strip:
Find the existing `.canvas-wrap` rule and update its `padding-bottom` or `bottom` to:
```css
padding-bottom: calc(100px + env(safe-area-inset-bottom, 0px));
```

- [ ] **Step 3: Add today-chip HTML to index.html**

Before the `</body>` tag, add:
```html
<button class="today-chip" id="today-chip" aria-label="返回今天">↩ 今天</button>
```

- [ ] **Step 4: Verify layout in browser**

Open `http://localhost:55303/` (or run `python3 -m http.server 55303` from `/Users/ericlu/DailyTree/app/`). Check:
- Timeline strip appears above bottom bar
- Bottom bar has 3 buttons: 统计 | + | 设置
- No layout overflow or overlap

- [ ] **Step 5: Commit**
```bash
git add app/index.html app/style.css
git commit -m "feat: add timeline strip above bottom nav, remove search button"
```

---

## Task 5: Timeline Strip + selectedDate Logic in app.js

**Files:**
- Modify: `app/app.js`

- [ ] **Step 1: Add selectedDate state and helpers near the top of app.js**

After the `const ONBOARDED_KEY = ...` line, add:

```js
// ── Date state ────────────────────────────────────────────────────────────
let selectedDate = getTodayKey(); // YYYY-MM-DD string
...
Add after `updateBottomBar` (around line 311):
...
function renderTimeline() {
  ...
  // Build 60 days: oldest first → today last
  const cells = [];
  for (let i = 59; i >= 0; i--) {
    const key = getDateKey(i);
    const [year] = key.split('-').map(Number);
    const hasEntry = (entries[year] || []).some(e => e.date.substring(0, 10) === key);
    cells.push({ key, hasEntry, isToday: key === todayKey });
  }

  cells.forEach(({ key, hasEntry, isToday: today }) => {
    ...
    dayEl.textContent = getShortDay(key);
  ...
  // Scroll today cell into view (centered)
...
function selectDate(dateStr) {
  ...
  if (chip) chip.classList.toggle('visible', !isToday(dateStr));

  if (!isToday(dateStr)) {
    // Show that day's entry in the branch panel
    const entries = loadEntries();
    const [year] = dateStr.split('-').map(Number);
    const entry = (entries[year] || []).find(e => e.date.substring(0, 10) === dateStr) || null;
    openDatePanel(dateStr, entry);
  } else {
    closeBranchPanel();
  }
}

function openDatePanel(dateStr, entry) {
  ...
  if (!entry) {
    ...
    li.textContent = '当天没有记录';
    list.appendChild(li);
  } else {
    const li = document.createElement('li');
    li.className = 'branch-entry';
    li.innerHTML = `<p class="branch-entry-text">${escapeHtml(entry.text)}</p>`;
    list.appendChild(li);
  }
...
- [ ] **Step 3: Update closeBranchPanel to also reset selectedDate**

Find `closeBranchPanel`:
```js
...
- [ ] **Step 4: Update refreshTree to call renderTimeline**

Find `function refreshTree()` and add `renderTimeline();`:
```js
...
```
(Remove the `updateBottomBar()` call since bar-chart divs are gone.)

- [ ] **Step 5: Bind today-chip click in bindEvents**

Find the `bindEvents` function. Add before the closing `}`:
```js
...
Also remove or comment out the `btn-search` event binding since that button no longer exists:
```js
// Remove these lines:
...
- Timeline strip shows 60 days
- Today is highlighted
- Days with entries show a green dot
- Tapping a past date with an entry opens the branch panel showing it
- "今天" chip appears when viewing past date
- Tapping "今天" chip or closing panel resets to today

- [ ] **Step 7: Commit**
```bash
git add app/app.js
git commit -m "feat: add timeline strip date navigation with selectedDate state"
```

---

## Task 6: Update app.js — New tree.js API + Branch Hit Detection

**Files:**
- Modify: `app/app.js`

The new `drawTree` signature is `drawTree(canvas, dayCount, seed)` (no vitality7) and returns `{ branchSegments }`. Update all callers.

- [ ] **Step 1: Add module-level branchSegments storage**

After the `let weatherManager = null;` line, add:
```js
let _branchSegments = []; // updated on each drawTree call
```

- [ ] **Step 2: Update startTreeAnimation**

Find and replace the entire `startTreeAnimation` function:
...
function startTreeAnimation() {
  ...
  if (!canvas) return;
  const total = computeTotalDaysThisYear();
  const seed  = new Date().getFullYear() * 1000 + total;
  ...
  if (stage === 'seed' || stage === 'crack' || stage === 'sprout') {
    ...
    treeAnimRafId = requestAnimationFrame(frame);
    _branchSegments = []; // early stages have no branches
  } else {
...
- [ ] **Step 3: Update onTreeClick to use branchSegments**

Find and replace the entire `onTreeClick` function:

```js
function onTreeClick(e) {
  if (_branchSegments.length === 0) return; // seed/sprout stages — no branches
  ...
  for (const seg of _branchSegments) {
    const d = ptSegDist(cx, cy, seg.x0, seg.y0, seg.x1, seg.y1);
    if (d < minDist) { minDist = d; closest = seg; }
  }

  if (closest && minDist < threshold) {
    openBranchPanel(closest.period);
  }
}
```

- [ ] **Step 4: Update the import line at top of app.js**
...
```
(`getTrunkBounds` is removed; it's no longer needed.)

- [ ] **Step 5: Update guide canvas drawTree call**
...
Replace with:
```js
drawTree(guideCanvas, 20, 42);
```
(Second arg is dayCount=20, third is seed=42.)

- [ ] **Step 6: Verify branch tapping**

With test data: add an entry so the tree reaches mature/ancient stage (or temporarily change `computeTotalDaysThisYear()` to return 100). Tap branches — the branch panel should open with 'early'/'mid'/'recent' period label.

- [ ] **Step 7: Commit**
```bash
git add app/app.js
git commit -m "feat: update tree hit detection to use branchSegments point-to-segment distance"
```

---

## Task 7: Sprouting Animation (first entry → stage transition)

**Files:**
- Modify: `app/app.js`

When a user submits their first entry of the day and the stage changes (seed→crack or crack→sprout), play a brief transition animation instead of an instant redraw.

- [ ] **Step 1: Add sproutingAnimation function**

Add after `showSaveSuccess`:

```js
function playSproutTransition(canvas, fromStage, toStage, seed) {
  // Animate progress 0→1 over 800ms, blending from→to stage
  ...
  function frame(now) {
    ...
    if (toStage === 'crack') {
      // Draw animated seed at full opacity, overlay crack fading in
      drawSeedAnimated_internal(ctx, W, H, now);
      ctx.globalAlpha = eased;
      drawCrack_internal(ctx, W, H);
      ctx.globalAlpha = 1;
    } else if (toStage === 'sprout') {
      // Draw crack, overlay sprout fading in
      drawCrack_internal(ctx, W, H);
      ctx.globalAlpha = eased;
      drawSprout_internal(ctx, W, H);
      ctx.globalAlpha = 1;
    }
    ...
    if (progress < 1) {
      ...
      // Transition complete — start normal animation loop
      startTreeAnimation();
    }
  }
  if (treeAnimRafId) { cancelAnimationFrame(treeAnimRafId); treeAnimRafId = null; }
...
Note: `drawSeedAnimated_internal`, `drawCrack_internal`, `drawSprout_internal` are the same as `drawSeedAnimated`, `drawCrack`, `drawSprout` in tree.js — but they can't be imported without exporting them. The simplest solution: add these three as additional named exports in tree.js, or use a single exported helper.

- [ ] **Step 2: Export internal draw functions from tree.js**

Add these exports at the bottom of `app/tree.js` (after the existing exports):
...
- [ ] **Step 3: Update import in app.js**

```js
import { drawTree, drawAnimated, getStage, _drawSeedAnimated, _drawCrackAnimated, _drawSprout } from './tree.js';
```

- [ ] **Step 4: Simplify playSproutTransition using imported functions**

Replace the `playSproutTransition` implementation with:

```js
function playSproutTransition(canvas, toStage) {
  const START = performance.now();
  const DURATION = 700;
  ...
  function frame(now) {
    ...
    if (toStage === 'crack') {
      _drawSeedAnimated(ctx, W, H, now);
      ctx.save(); ctx.globalAlpha = eased;
      // draw just the crack overlay
      const cx = W / 2, cy = H * 0.62;
      ctx.beginPath(); ctx.moveTo(cx, cy - 12); ctx.lineTo(cx - 2, cy - 22); ctx.lineTo(cx + 1, cy - 28);
      ctx.strokeStyle = 'rgba(20,10,4,0.9)'; ctx.lineWidth = 1.4; ctx.lineCap = 'round'; ctx.stroke();
      ctx.beginPath(); ctx.ellipse(cx, cy - 29, 2.5, 4, 0.1, 0, Math.PI * 2);
      ...
      _drawCrackAnimated(ctx, W, H, now);
      ctx.save(); ctx.globalAlpha = eased;
      _drawSprout(ctx, W, H);
      ctx.restore();
    }
...
- [ ] **Step 5: Trigger transition in submitEntry**

Find `function submitEntry()`. After `closeModal(); refreshTree();`, update:
...
function submitEntry() {
  ...
  const prevStage = getStage(prevTotal);
  ...
  const shouldAnimate = canvas &&
    prevStage !== newStage &&
    (newStage === 'crack' || newStage === 'sprout');

  if (shouldAnimate) {
    playSproutTransition(canvas, newStage);
  } else {
    refreshTree();
  }
}
...
To test without real data: temporarily change `addEntryWithDate` call or set dayCount=0, then add entry. Should see crack/sprout animation. Restore after testing.

- [ ] **Step 7: Commit**
```bash
git add app/tree.js app/app.js
git commit -m "feat: add sprouting transition animation when stage changes on first entry"
```
...
Open the app. Verify:
1. Landing page → no flash (loads landing.html directly)
2. App opens → timeline strip shows 60 days, today highlighted
3. Tree shows correct stage for current dayCount
4. Tap "+" → modal opens → enter text → save → tree re-renders (or animates if stage changes)
5. Tap a past date in timeline → branch panel shows that day's entry (or "no entry")
6. "今天" chip appears → tapping it resets to today
7. Tap a branch on mature/ancient tree → branch panel opens with period label

- [ ] **Step 2: Remove dead code from app.js**

Remove `openSearch`, `closeSearch`, `renderSearchResults` functions and the search overlay HTML from `index.html` (or leave search overlay HTML since it may still be linked from settings). At minimum remove the `btn-search` binding.

Remove `updateBottomBar` function (bar-chart divs are gone).
...
Wait for GitHub Pages deployment, then test on mobile (iOS Safari) — verify:
- No iOS auto-zoom on textarea (font-size ≥16px already fixed in prior branch)
- Timeline scrolls horizontally on touch
- Branch tap works with touch (18px threshold)

- [ ] **Step 4: Final commit if any fixes**
```bash
git add -A
git commit -m "fix: post-integration cleanup and mobile verification"
git push origin main
```
